...
# 简单读取文件
def read_tif(file_path: str) -> (np.ndarray, tuple, str):
...
# 剔除3σ外的数，返回合理的最值，用于写入新tiff
def extreme_value(array_data):
    # 计算均值和标准差
    ...
    # 剔除异常值
    new_data = np.where((array_data < lower_bound) | (array_data > upper_bound), np.nan, array_data)
    max_value = np.nanmax(new_data)
    min_value = np.nanmin(new_data)
    del array_data, mean, std, lower_bound, upper_bound, new_data
...
def write_tif(save_path: str, data_: np.ndarray, geotransform_: tuple, projection_: str):
    ...
    max_value, min_value = extreme_value(data_)
