...
def main(vi_):
    ...
    for i in range(len(path)):
        if i < 9:
            date.append(os.path.join(disk1, path[i]))
        else:
            date.append(os.path.join(disk2, path[i]))

    max_, min_, extreme = np.array([]), np.array([]), np.array([])  # 创建空数组
    for day in date:
        print('Getting extreme of ' + day[3:])
        mask = roi_mask(day, vi_)
        arr = tt.readTiffArray(os.path.join(day, "VIs", vi_ + r".tif"))
    ...
    for day in date:
        ...
        label = day[8:]
        tc.map_array(roi, extreme, color, label, os.path.join("E:", "VI" + vi_, day[7:] + vi_ + r".png"))

    generate_gif(os.path.join("F:", "VI", vi_))
